<template>
  <div class="text-center mt-10">
    <v-card class="pa-5 ml-5 mr-5 blue lighten-5">
      <h1 class="text-center mb-5">Ingreso de Pacientes</h1>
        <v-text-field label="Nombre" type="text" :value="currentPatient.data.name" @input="updateName" outlined color="blue darken-1"/>
        <v-text-field label="Email" type="text" :value="currentPatient.data.email" @input="updateEmail" outlined color="blue darken-1"/>  
      <v-divider></v-divider>
      <v-card-actions>
        <v-spacer></v-spacer>
